...
export interface Fetcher {
  (gql: Gql, fetchImpl: typeof fetch): Promise<ExecutionResult<any>>
}
...
export const createClient = <QR, Q, MR, M, SR, S>({
  ...
  return {
    query: request => {
      ...
      return fetcher(requestToGql('query', queryRoot, <any>request), fetch)
    },
    mutation: request => {
      if (!fetcher) throw new Error('fetcher argument is missing')
      if (!mutationRoot) throw new Error('mutationRoot argument is missing')
      return fetcher(requestToGql('mutation', mutationRoot, <any>request), fetch)
    },
